...
if manifest_match:
    ...
    if app_uuid in manifest:
        ...
        # Replace the hardcoded heights with clamp
        text = text.replace('height: "65px"', 'height: "clamp(35px, 8vw, 65px)"')
        text = text.replace('height: "45px"', 'height: "clamp(24px, 5.5vw, 45px)"')
        text = text.replace('gap: "10px"', 'gap: "clamp(4px, 1.5vw, 10px)"')
...
idx_html = idx_html.replace('height: 65px;', 'height: clamp(35px, 8vw, 65px);')
idx_html = idx_html.replace('height: 45px;', 'height: clamp(24px, 5.5vw, 45px);')
idx_html = idx_html.replace('gap: 10px;', 'gap: clamp(4px, 1.5vw, 10px);')

with open('index.html', 'w', encoding='utf-8') as out:
    out.write(idx_html)

print("Successfully applied clamp() to logo styles.")
