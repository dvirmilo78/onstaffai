...
NEW_CHARTS_ANIM = r"""const ChartsAnim = ({ lang }) => {
  ...
  const yBase   = barAreaH - 28;
  const maxBarH = yBase - 48;

  // Feed card constants
  const feedX = 524, feedY = 200, feedW = 262, feedH = 232;
  const rowH = 50, feedTopPad = 50;
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%"
      ...
      {kpiItems.map((k, i) => (
        <g key={i} transform={`translate(${k.x}, 8)`}>
          ...
          {/* label centered */}
          <text x={kpiW/2} y="32" textAnchor="middle" fontSize="13" fill={ink} opacity=".65"
                fontFamily="'Heebo',sans-serif" fontWeight="600">{k.label}</text>
          {/* big value centered */}
          <text x={kpiW/2} y="62" textAnchor="middle" fontSize="26" fontWeight="900" fill={k.color}
                fontFamily="'Space Grotesk',sans-serif" letterSpacing="-0.02em"
                direction="ltr">{k.val}</text>
          {/* live badge centered */}
          <rect x={kpiW/2 - 28} y="68" width="56" height="16" rx="8" fill={k.color} opacity="0.15"/>
          <text x={kpiW/2} y="79" textAnchor="middle" fontSize="10" fontWeight="800"
      ...
      <g transform={`translate(${barAreaX}, ${barAreaY - 10})`}>
        <rect width={barAreaW + 4} height={barAreaH + 36} rx="14"
              fill={card} stroke={orange} strokeWidth="1" strokeOpacity="0.25"/>
        <circle cx="16" cy="20" r="5" fill={green}>
          <animate attributeName="opacity" values="1;0.3;1" dur="1.5s" repeatCount="indefinite"/>
        </circle>
        <text x={(barAreaW+4)/2} y="25" textAnchor="middle" fontSize="14" fontWeight="700" fill={ink}
              fontFamily="'Heebo',sans-serif">
          {isHe ? "משימות שהושלמו לפי סוכן" : "Tasks completed by agent"}
        </text>
        <rect x={barAreaW - 46} y="9" width="50" height="20" rx="10" fill={orange} opacity="0.15"/>
        <text x={barAreaW - 21} y="23" textAnchor="middle" fontSize="10" fontWeight="800"
        ...
        {agentTasks.map((h, i) => {
          ...
          return (
            <g key={i}>
              ...
              <rect x={x + barW/2 - 20} y={barTop - 22} width="40" height="18" rx="9"
                    fill={isPeak ? orange : card} fillOpacity={isPeak ? 0.2 : 0.7}/>
              <text x={x + barW/2} y={barTop - 9}
              ...
              {/* agent name fixed below all bars */}
              <text x={x + barW/2} y={40 + maxBarH + 20} textAnchor="middle"
                    fontSize="11" fill={isPeak ? orange : ink} opacity={isPeak ? 1 : 0.6}
      ...
      {/* ── Live feed card — all text centered ── */}
      <g transform={`translate(${feedX}, ${feedY})`}>
        <rect width={feedW} height={feedH} rx="14"
              fill={card} stroke={orange} strokeWidth="1.5" strokeOpacity="0.5"/>

        {/* header centered */}
        <circle cx={feedW/2} cy="16" r="5" fill={orange}>
          <animate attributeName="opacity" values="1;0.3;1" dur="1.2s" repeatCount="indefinite"/>
        </circle>
        <text x={feedW/2} y="36" textAnchor="middle" fontSize="15" fontWeight="800" fill={ink}
              fontFamily="'Heebo',sans-serif">
          {isHe ? "פעילות בזמן אמת" : "Real-time feed"}
        </text>

        {feed.map((f, i) => {
          const rowY = feedTopPad + i * rowH;
          const cx = feedW / 2;  // center x of card
          return (
            <g key={i} opacity={1 - i * 0.15}>
              {/* solid dark bg */}
              <rect x="8" y={rowY} width={feedW - 16} height={rowH - 4} rx="10" fill={f.bg}/>
              {/* left accent bar */}
              <rect x="8" y={rowY} width="5" height={rowH - 4} rx="3" fill={f.color}/>
              {/* avatar — centered */}
              <circle cx={cx} cy={rowY + 14} r="12" fill={f.color}/>
              <text x={cx} y={rowY + 19} textAnchor="middle" fontSize="13" fill="#fff"
                    fontFamily="'Heebo',sans-serif" fontWeight="800">
                {f.agent.charAt(0)}
              </text>
              {/* agent name centered below avatar */}
              <text x={cx} y={rowY + 32} textAnchor="middle" fontSize="13" fontWeight="800"
                    fill={f.color} fontFamily="'Heebo',sans-serif">{f.agent}</text>
              {/* action centered below name */}
              <text x={cx} y={rowY + 44} textAnchor="middle" fontSize="12" fill="#e2e8f0"
      ...
      <g transform="translate(16, 450)">
        ...
        <text x="384" y="18" textAnchor="middle" fontSize="12" fontWeight="600" fill={ink} opacity=".6"
              fontFamily="'Heebo',sans-serif">
          {isHe ? "💰 הכנסות מצטברות — היום" : "💰 Revenue generated today"}
        </text>
        <text x="384" y="38" textAnchor="middle" fontSize="21" fontWeight="900"
