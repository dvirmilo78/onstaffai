...
NEW_CHARTS_ANIM = r"""const ChartsAnim = ({ lang }) => {
  ...
  // Feed layout — RTL: avatar on RIGHT, text centered in left portion
  const feedX = 522, feedY = 198, feedW = 266, feedH = 238;
  const rowH = 52, feedTopPad = 52;
  // avatar sits at right side of each row
  const avatarCX = feedW - 28;  // right side
  // text is centered in the remaining left area
  const textCX = (feedW - 56) / 2 + 10;  // center of left portion
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%"
      ...
      {kpiItems.map((k, i) => (
        <g key={i} transform={`translate(${k.x}, 8)`}>
          ...
          <circle cx={kpiW/2} cy="14" r="6" fill={k.color}>
      ...
      <g transform={`translate(${barAreaX}, ${barAreaY - 10})`}>
        <rect width={barAreaW + 4} height={barAreaH + 38} rx="14"
              fill={card} stroke={orange} strokeWidth="1" strokeOpacity="0.25"/>
        <circle cx="16" cy="20" r="5" fill={green}>
          <animate attributeName="opacity" values="1;0.3;1" dur="1.5s" repeatCount="indefinite"/>
        </circle>
        <text x={(barAreaW+4)/2} y="26" textAnchor="middle" fontSize="14" fontWeight="700" fill={ink}
      ...
      {/* ── Live feed card — RTL: avatar RIGHT, text centered in left portion ── */}
      <g transform={`translate(${feedX}, ${feedY})`}>
        ...
        {/* header — centered */}
        <circle cx={feedW/2} cy="16" r="6" fill={orange}>
          <animate attributeName="opacity" values="1;0.3;1" dur="1.2s" repeatCount="indefinite"/>
        </circle>
        <text x={feedW/2} y="38" textAnchor="middle" fontSize="16" fontWeight="800" fill={ink}
              fontFamily="'Heebo',sans-serif">
        ...
        {feed.map((f, i) => {
          ...
          return (
            <g key={i} opacity={1 - i * 0.15}>
              ...
              {/* agent name centered in left text area */}
              ...
              {/* action centered in left text area */}
